...
def check_pam_faillock_enabled():
    ...
    Kontroller:
      1) common-auth içinde 'auth ... requisite ... pam_faillock.so ... preauth'
      2) common-auth içinde 'auth ... [default=die] ... pam_faillock.so ... authfail'
      3) common-account içinde 'account ... required ... pam_faillock.so'
    """
    logger.info("[CIS 5.3.2.2][CHECK] PAM faillock modülü kontrol ediliyor...")
    ...
    try:
        ...
        checks = {
            "preauth": {
                ...
                "regex": r'^\s*auth\s+requisite\s+pam_faillock\.so\s+preauth'
            },
            "authfail": {
                "file": "/etc/pam.d/common-auth",
                "regex": r'^\s*auth\s+\[default=die\]\s+pam_faillock\.so\s+authfail'
            },
            "account": {
                "file": "/etc/pam.d/common-account",
                "regex": r'^\s*account\s+required\s+pam_faillock\.so'
            }
        }
        ...
        for key, data in checks.items():
            cmd = ["grep", "-P", data["regex"], data["file"]]
            success, output = run_command(cmd)
            
            if success:
                results[key] = True
            else:
                missing_items.append(f"{key} ({data['file']})")

        if all(results.values()):
            logger.info("[CIS 5.3.2.2][CHECK] Başarılı: pam_faillock tüm kuralları mevcut.")
            return True, "pam_faillock etkin ve yapılandırılmış."
        else:
            msg = "Eksik konfigürasyonlar: " + ", ".join(missing_items)
            logger.warning(f"[CIS 5.3.2.2][CHECK] Başarısız: {msg}")
            return False, msg

    except Exception as e:
        logger.error(f"[CIS 5.3.2.2][CHECK] Hata: {str(e)}")
...
def apply_pam_faillock(username=None, param=None):
    """
    CIS 5.3.2.2 - Ensure pam_faillock module is enabled
    Debian/Pardus için /usr/share/pam-configs altına profilleri yazar ve etkinleştirir.
    """
    logger.info("[CIS 5.3.2.2][APPLY] PAM faillock yapılandırması uygulanıyor...")
    ...
    try:
        ...
        path_faillock = os.path.join(config_dir, "faillock")
        path_notify = os.path.join(config_dir, "faillock_notify")

        with open(path_faillock, "w", encoding="utf-8") as f:
            f.write(faillock_content)
        
        with open(path_notify, "w", encoding="utf-8") as f:
            f.write(faillock_notify_content)

        logger.info("[APPLY] Profil dosyaları oluşturuldu. pam-auth-update çalıştırılıyor...")

        # PAM profillerini etkinleştirir
        cmd_update = ["pam-auth-update", "--enable", "faillock", "faillock_notify"]
        
        success, output = run_command(cmd_update)
        
        if not success:
            logger.error(f"[APPLY] pam-auth-update hatası: {output}")
            return False, f"PAM güncelleme hatası: {output}"

        is_ok, check_msg = check_pam_faillock_enabled()
        
        if is_ok:
            return True, "pam_faillock başarıyla yapılandırıldı ve etkinleştirildi."
        else:
            return False, f"Yapılandırma uygulandı fakat doğrulama başarısız: {check_msg}"

    except Exception as e:
        logger.error(f"[CIS 5.3.2.2][APPLY] Kritik Hata: {str(e)}")
        return False, f"İşlem sırasında istisna oluştu: {str(e)}"



def disable_pam_faillock(username=None, param=None):
    """
    CIS 5.3.2.2 değişikliğini güvenli bir şekilde geri alır.
    
    Yöntem:
    1. pam-auth-update --remove ile profilleri deaktif eder (Config dosyalarını sistemin kendisi düzenler).
    2. /usr/share/pam-configs/ altındaki oluşturulan dosyaları temizler.
    3. pam_faillock referanslarının kalkıp kalkmadığını kontrol eder.
    """
    logger.info("[CIS 5.3.2.2][DISABLE] PAM faillock devre dışı bırakılıyor...")

    try:
        # PAM Update aracıyla profilleri güvenli şekilde sistemden dusurur.
        # Bu komut common-auth ve common-account dosyalarını yeniden yazar ama faillock satırlarını çıkarır.
        cmd_remove = ["pam-auth-update", "--remove", "faillock", "faillock_notify"]
        
        logger.info("[DISABLE] pam-auth-update --remove çalıştırılıyor...")
        success, output = run_command(cmd_remove)

        if not success:
            logger.error(f"[DISABLE] PAM update hatası: {output}")
            return False, f"PAM yapılandırması kaldırılamadı: {output}"

        # Profil dosyalarını diskten temizler
        config_dir = "/usr/share/pam-configs"
        files_to_remove = ["faillock", "faillock_notify"]
        removed_count = 0
        ...
        for fname in files_to_remove:
            ...
            if os.path.exists(fpath):
                try:
                    os.remove(fpath)
                    removed_count += 1
                    logger.info(f"[DISABLE] Dosya silindi: {fpath}")
                except OSError as e:
                    logger.warning(f"[DISABLE] Dosya silinemedi ({fpath}): {e}")
            else:
                logger.info(f"[DISABLE] Dosya zaten yok: {fpath}")

        run_command(["pam-auth-update", "--package"])

        check_cmd = ["grep", "-r", "pam_faillock.so", "/etc/pam.d/common-auth", "/etc/pam.d/common-account"]
        found, grep_output = run_command(check_cmd)

        if found and grep_output.strip():
            logger.warning(f"[DISABLE] UYARI: pam_faillock hala config dosyalarında görünüyor:\n{grep_output}")
            return False, "İşlem yapıldı ancak pam_faillock.so hala config dosyalarında mevcut. Manuel kontrol gerekebilir."
        
        logger.info("[CIS 5.3.2.2][DISABLE] İşlem başarılı. Faillock modülü tamamen kaldırıldı.")
        return True, "pam_faillock başarıyla devre dışı bırakıldı ve dosyalar temizlendi."

    except Exception as e:
        logger.error(f"[CIS 5.3.2.2][DISABLE] Kritik Hata: {str(e)}")
        return False, f"Geri alma sırasında hata: {str(e)}"
...
def check_failed_attempts_lockout(expected_deny=DEFAULT_DENY):
    ...
    Kontroller:
      ✔ faillock.conf içinde deny = 1–5 aralığında olmalı
      ✔ common-auth içinde deny >= 6 olmamalı
    """
    try:
        ...
        # common-auth içinde yanlış deny parametresi var mı?
        cmd = [
            "grep", "-Pi",
        ...
        logger.info(f"[CIS 5.3.3.1.1][CHECK] deny={deny_value}, CIS uyumlu.")
        return True, f"deny={deny_value}, CIS uyumlu."
...
def apply_failed_attempts_lockout(username=None, param=None):
    """
    CIS 5.3.3.1.1 - Full remediation
      ✔ faillock.conf deny=5 yapar
      ✔ common-auth içinden deny kaldırır
      ✔ /usr/share/pam-configs/* içindeki pam_faillock deny parametrelerini kaldırır (CRITICAL)
    """
    try:
        param = param or {}
        expected_deny = int(param.get("deny", DEFAULT_DENY))

        ok, msg = check_failed_attempts_lockout(expected_deny)
        if ok:
            logger.info(f"[CIS 5.3.3.1.1][APPLY] Ayar zaten uygun: {msg}")
            return True, msg

        logger.warning(f"[CIS 5.3.3.1.1][APPLY] Düzenleme başlatılıyor: {msg}")

        # 1) faillock.conf içinde deny satırını düzenle
        logger.info("[CIS 5.3.3.1.1][APPLY] faillock.conf deny=5 olarak ayarlanıyor...")
        run_command(["sed", "-Ei", r"s/^\s*deny\s*=.*/deny = 5/", FAILLOCK_CONF])

        # deny satırı yoksa ekle
        run_command(["bash", "-c", f"grep -Pq '^\\s*deny\\s*=' {FAILLOCK_CONF} || echo 'deny = 5' >> {FAILLOCK_CONF}"])

        # 2) common-auth içindeki deny parametresini kaldır
        logger.info("[CIS 5.3.3.1.1][APPLY] common-auth deny temizleniyor...")
        run_command([
            "sed", "-Ei",
            r"/pam_faillock.so/ s/\s*deny=[0-9]+//g",
            "/etc/pam.d/common-auth"
        ])

        # 3) /usr/share/pam-configs/* içindeki deny parametrelerini temizle
        logger.info("[CIS 5.3.3.1.1][APPLY] PAM config paketleri taranıyor (deny temizlenecek)...")
        ...
        for fname in os.listdir(PAM_CONFIG_DIR):
            ...
            # dosyada deny= var mı?
            ok, out = run_command(["grep", "-Pi", r"pam_faillock\.so.*\bdeny=", path])

            if ok and out.strip():
                logger.warning(f"[CIS 5.3.3.1.1][APPLY] deny bulundu, temizleniyor: {path}")
                run_command([
                    "sed", "-Ei",
                    r"s/(pam_faillock\.so[^#]*?)\s*deny=[0-9]+/\1/g",
                    path
                ])

        # PAM databasenin yeniden oluşturulması (Debian için önemli)
        run_command(["pam-auth-update", "--package"])

        # Final doğrulama
        ok, msg = check_failed_attempts_lockout(expected_deny)
        if ok:
            logger.info("[CIS 5.3.3.1.1][APPLY] Başarıyla uygulandı.")
            return True, f"Başarıyla uygulandı: {msg}"
        else:
            logger.error(f"[CIS 5.3.3.1.1][APPLY] Doğrulama başarısız: {msg}")
            return False, msg

    except Exception as e:
        logger.error(f"[CIS 5.3.3.1.1][APPLY] Hata: {e}")
        return False, f"Hata: {e}"
...
def apply_root_account_lock(username=None, param=None):
    ...
    - even_deny_root ekler
    - root_unlock_time varsa >=60 olacak şekilde günceller veya 0 ise kaldırır (CIS gereği)
    - PAM içindeki root_unlock_time argümanlarını temizler
    - PAM config dosyaları düzenlendikten sonra pam-auth-update çağrılır (CIS gereği)
    """
    try:
        param = param or {}
        ...
        logger.warning(f"[CIS 5.3.3.1.3][APPLY] Düzenleme başlıyor: {msg}")

        # 1) Backup
        shutil.copy2(FAILLOCK_CONF, FAILLOCK_CONF + ".bak")

        # 2) FAILLOCK_CONF düzenleme (CIS: update OR remove)
        lines = []
        has_even = False
        root_time_removed = False

        with open(FAILLOCK_CONF, "r") as f:
            for line in f:
                stripped = line.strip()

                # even_deny_root → her zaman olmalı
                if re.match(r"^\s*even_deny_root\b", stripped):
                    has_even = True
                    lines.append("even_deny_root\n")
                    continue

                # root_unlock_time → 0 ise kaldır, 1–59 ise ≥60 yap
                if re.match(r"^\s*root_unlock_time\s*=", stripped):
                    val = _safe_int(stripped.split("=")[1].strip(), 0)

                    # CIS: remove or update
                    if val == 0:
                        # 0 → tamamen kaldırılacak
                        root_time_removed = True
                        continue
                    elif val < 60:
                        # update >= 60
                        ...
                        # zaten >= 60, olduğu gibi ekle
                        lines.append(line)
                        continue

                lines.append(line)

        # even_deny_root yoksa ekle (CIS zorunlu)
        if not has_even:
            lines.append("\neven_deny_root\n")

        # root_unlock_time hiç yoksa ekleme zorunlu değil (CIS: “if set”)
        with open(FAILLOCK_CONF, "w") as f:
            f.writelines(lines)

        # 3) PAM config dosyalarındaki root_unlock_time temizle
        ok2, files = run_command([
            "grep", "-Pl",
            r"\bpam_faillock\.so.*root_unlock_time\b",
            PAM_CONFIG_DIR
        ])

        if ok2 and files.strip():
            for fpath in files.splitlines():
                logger.warning(f"[CIS 5.3.3.1.3][APPLY] PAM config düzeltme: {fpath}")

                with open(fpath, "r") as pf:
                    txt = pf.read().splitlines()

                cleaned = []
                for ln in txt:
                    # root_unlock_time=<x> parametresini kaldırıyoruz
                    parts = [p for p in ln.split() if not p.startswith("root_unlock_time=")]
                    cleaned.append(" ".join(parts))

                with open(fpath, "w") as pf:
                    pf.write("\n".join(cleaned) + "\n")

        # 4) CIS gereği → pam-auth-update çalıştırılacak
        #    COMMON-AUTH yeniden oluşturulmazsa değişiklik uygulanmaz.
        logger.info("[CIS 5.3.3.1.3][APPLY] pam-auth-update çalıştırılıyor...")
        run_command(["pam-auth-update", "--package"])
...
def apply_maxrepeat(username=None, param=None):
    ...
    try:
        ...
        logger.info("[CIS 5.3.3.2.4][APPLY] pam-auth-update çalıştırılıyor...")
        run_command(["pam-auth-update", "--package"])
