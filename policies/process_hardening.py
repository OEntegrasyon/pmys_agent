    ...
    try:
        ...
        if success:
            ...
            if current_value == expected_value:
                return True, f"{key} çalışan değeri '{expected_value}' olarak doğru ayarlanmış."
            else:
                print(f"Denetim: {key} çalışan değeri yanlış (Beklenen: {expected_value}, Mevcut: {current_value}). Düzeltme uygulanacak.")
                return apply_aslr_enabled(parameters)
        else:
            return False, f"sysctl {key} komutu çalıştırılamadı: {output}"
    ...
    try:
        ...
        if success:
            ...
            if current_value in allowed_values:
                return True, f"{key} çalışan değeri '{current_value}' (Uyumlu)."
            else:
                print(f"Denetim: {key} çalışan değeri yanlış (Beklenen: {allowed_values}, Mevcut: {current_value}). Düzeltme uygulanacak.")
...
def apply_ptrace_scope(parameters):
    ...
    try:
        with open(temp_path, "w") as f:
            f.write(f"# Agent tarafından CIS 1.5.2 politikası için ayarlandı\n")
            f.write(f"{key} = {value}\n")
...
def restrict_core_dumps(username, parameters):
    ...
    if os.path.isdir(limits_d_path):
        files_to_check.extend(glob.glob(os.path.join(limits_d_path, "*.conf")))

    try:
        limits_regex = re.compile(r"^\s*\*\s+hard\s+core\s+0")
        
        for file_path in files_to_check:
            if os.path.exists(file_path):
                with open(file_path, "r") as f:
                    content = f.read()
                    if limits_regex.search(content, re.MULTILINE):
    ...
    if sysctl_running_ok and limits_durable_ok:
        ...
        if not limits_durable_ok: print(f"Denetim: '/etc/security/limits.conf' veya '.d/' içinde '* hard core 0' kuralı bulunamadı.")
...
def apply_core_dumps(parameters):
    ...
    try:
        ...
        with open(temp_path, "w") as f:
            f.write(f"# Agent tarafından CIS 1.5.3 politikası için ayarlandı\n")
            f.write(f"{key} = {value}\n")
